...
Step 3: TOTVS MCPs → language-system-docs-search (FW*/TC*/MS*/AdvPL/TLPP classes)
                     code-search (real usage examples in the codebase)
                     tir-docs-search (TIR Python)
Step 4: Web search → TDN (tdn.totvs.com), official docs, trusted sources
Step 5: Flag as uncertain → "I'm not sure about X — this is my reasoning, but verify"
```
...
- **Validate ALL external symbols** (classes, methods, functions, namespaces) in the MCPs before writing calls — see symbol validation rules in AGENTS.md

...
3. DO NOT proceed to the gate check without this conversion — accented characters silently corrupt at compile time
...
2. **Compilation without errors** — Compile via Language Server (VS Code) or TOTVS Language Server CLI. Zero compilation errors is the gate condition. Compilation errors = STOP, fix, recompile.

...
export class SyncMasterDataService {
  ...
  async syncEstados(conteudo: any[]) {
    return this.syncBatch(Estado, conteudo);
  }
  async syncMunicipios(conteudo: any[]) {
    return this.syncBatch(Municipio, conteudo, { estado: 'estado' });
  }
  async syncFiliais(conteudo: any[]) {
    return this.syncBatch(Filial, conteudo);
  }
  async syncClientes(conteudo: any[]) {
    return this.syncBatch(Cliente, conteudo, {
      vendedor: 'vendedor',
  ...
  async syncVendedores(conteudo: any[]) {
    return this.syncBatch(Vendedor, conteudo);
  }
  async syncTabelasPreco(conteudo: any[]) {
    return this.syncBatch(TabelaPreco, conteudo);
  }
}

...
export class ModuleListComponent implements OnInit {
  ...
  readonly actions: Array<PoPageAction> = [
    { label: "Novo Módulo", action: () => this.router.navigate(["/admin/modules/new"]), icon: "an an-plus" }
  ];

  readonly tableActions: Array<PoTableAction> = [
    { label: "Editar", action: (row: any) => this.router.navigate([`/admin/modules/edit/${row.id}`]), icon: "an an-pencil-simple" },
    { label: "Excluir", action: this.deleteModule.bind(this), icon: "an an-trash", type: "danger" }
  ];

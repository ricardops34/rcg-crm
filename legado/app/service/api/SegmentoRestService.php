...
class SegmentoRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'Segmento';
    const ATTRIBUTES    = ['cod_erp','descricao','dt_alteracao','dt_inclusao','id','reg_ativo','status'];
    ...
    public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = Segmento::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                $aGrava['id'] = $oBusca->id;
            }

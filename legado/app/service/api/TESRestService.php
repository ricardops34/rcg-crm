...
class TESRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'TipoEntradaSaida';
    const ATTRIBUTES    = ['cfop','cod_erp','descricao','dt_alteracao','dt_inclusao','empresa_id','filial_id','finalidade','id','status','system_unit_id','tipo'];
        ...
        public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = TipoEntradaSaida::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                if($oBusca){
                    $aGrava['id'] = $oBusca->id;
                }

...
class FabricanteRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'Fabricante';
    const ATTRIBUTES    = ['cod_erp','descricao','dt_alteracao','dt_inclusao','id'];
        ...
        public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = Fabricante::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                $aGrava['id'] = $oBusca->id;
            }

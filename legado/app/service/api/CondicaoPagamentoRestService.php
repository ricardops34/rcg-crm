...
class CondicaoPagamentoRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'CondicaoPagamento';
    const ATTRIBUTES    = ['cod_erp','descricao','dt_inclusao','filial_id','id'];
        ...
        if( isset( $aRequest['conteudo'] ) ){
            ...
            foreach ($aItens as $aItem) {
                ...
                if(isset($aGrava['cod_erp'])){
                    $oBusca = CondicaoPagamento::where('cod_erp', '=', $aGrava['cod_erp'])
                    ->first(); 
                    $aGrava['id'] = $oBusca->id;
                }

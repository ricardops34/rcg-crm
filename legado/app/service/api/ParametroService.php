...
class ParametroService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'Parametro';
    const ATTRIBUTES    = ['conteudo','filial_id','id','parametro','tipo'];
    ...
    public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['parametro'])){
                $oBusca = Parametro::where('parametro', '=', $aGrava['parametro'])
                ->first(); 
                if($oBusca){
                    $aGrava['id'] = $oBusca->id;
                }

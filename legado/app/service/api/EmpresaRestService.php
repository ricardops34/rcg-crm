...
class EmpresaRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'Empresa';
    const ATTRIBUTES    = ['cod_erp','dt_alteracao','dt_inclusao','id','logo','nome','status','system_unit_id'];
    ...
    public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = Armazem::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                $aGrava['id'] = $oBusca->id;
            }

...
class RegiaoRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'RegiaoCliente';
    const ATTRIBUTES    = ['cod_erp','cor','descricao','dt_alteracao','dt_inclusao','id'];
    ...
    public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = RegiaoCliente::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                $aGrava['id'] = $oBusca->id;
            }

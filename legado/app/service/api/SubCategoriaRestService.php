...
class SubCategoriaRestService extends AdiantiRecordService
{
    const DATABASE      = 'erp_online';
    const ACTIVE_RECORD = 'SubCategoria';
    const ATTRIBUTES    = ['categoria_id','cod_erp','descricao','dt_alteracao','dt_inclusao','id','status'];
        ...
        public function StoreArray($request)
    {
        ...
        foreach ($aItens as $aItem) {   
            ...
            if(isset($aGrava['cod_erp'])){
                $oBusca = SubCategoria::where('cod_erp', '=', $aGrava['cod_erp'])
                ->first(); 
                $aGrava['id'] = $oBusca->id;
            }

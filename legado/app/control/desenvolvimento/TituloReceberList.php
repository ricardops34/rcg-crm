...
class TituloReceberList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'form_TituloReceberList';
    ...
    public function __construct($param = null)
    {
        ...
        $this->form->setFormTitle("Listagem de titulo recebers");
        $this->limit = 20;

        $criteria_cliente_id = new TCriteria();
        $criteria_vendedor_id = new TCriteria();

        $cliente_id = new TDBCombo('cliente_id', 'erp_online', 'Cliente', 'id', '{razao}','razao asc' , $criteria_cliente_id );
        ...
        $valor->setSize('100%');
        $numero->setSize('100%');
        $venc_real->setSize(110);
        $parcela->setSize('100%');
        $cliente_id->setSize('100%');
        $vendedor_id->setSize('100%');

        $row1 = $this->form->addFields([new TLabel("Cliente:", null, '14px', null, '100%'),$cliente_id],[new TLabel("Vendedor:", null, '14px', null, '100%'),$vendedor_id]);
        $row1->layout = ['col-sm-6','col-sm-6'];

        $row2 = $this->form->addFields([new TLabel("Emissão:", null, '14px', null, '100%'),$emissao],[new TLabel("Numero:", null, '14px', null, '100%'),$numero]);
        $row2->layout = ['col-sm-6','col-sm-6'];

        $row3 = $this->form->addFields([new TLabel("Parcela:", null, '14px', null, '100%'),$parcela],[new TLabel("Valor:", null, '14px', null, '100%'),$valor]);
        $row3->layout = ['col-sm-6','col-sm-6'];

        $row4 = $this->form->addFields([new TLabel("Vencimento Real:", null, '14px', null, '100%'),$venc_real],[]);
        ...
        $btn_onshow = $this->form->addAction("Cadastrar", new TAction(['TituloReceberForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $column_venc_real = new TDataGridColumn('venc_real', "Vencimento Real", 'left');
        ...
        $this->datagrid->addColumn($column_venc_real);

        $action_onEdit = new TDataGridAction(array('TituloReceberForm', 'onEdit'));
        ...
        $action_onView->setImage('fas:address-card #4CAF50');
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['TituloReceberList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['TituloReceberList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['TituloReceberList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['TituloReceberList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Desenvolvimento","Titulo recebers"]));
        }
    ...
    public function onSearch($param = null)
    {
        ...
        if (isset($data->emissao) AND ( (is_scalar($data->emissao) AND $data->emissao !== '') OR (is_array($data->emissao) AND (!empty($data->emissao)) )) )
        {

            $filters[] = new TFilter('emissao', '=', $data->emissao);// create the filter 
        }
        ...
        if (isset($data->venc_real) AND ( (is_scalar($data->venc_real) AND $data->venc_real !== '') OR (is_array($data->venc_real) AND (!empty($data->venc_real)) )) )
        {

            $filters[] = new TFilter('venc_real', '=', $data->venc_real);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for TituloReceber

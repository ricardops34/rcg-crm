...
class ClienteNotafiscalSimpleList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $column_vlr_itens_transformed = new TDataGridColumn('vlr_itens', "Valor", 'right');

        $column_vlr_itens_transformed->setTotalFunction( function($values) { 
        ...
        $column_vlr_itens_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!$value)
            {
                $value = 0;
        ...
        $this->datagrid->addColumn($column_vlr_itens_transformed);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Gerencia","ClienteNotafiscalSimpleList"]));
        }
        $container->add($panel);
    ...
    public function onReload($param = NULL)
    {
        ...
        {
            TSession::setValue(__CLASS__.'load_filter_vendedor1_id', $param["vendedor_id"] ?? "");
        }
        $filterVar = TSession::getValue(__CLASS__.'load_filter_vendedor1_id');
            $criteria->add(new TFilter('vendedor1_id', '=', $filterVar));

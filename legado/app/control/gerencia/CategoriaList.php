...
class CategoriaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $column_usado_transformed = new TDataGridColumn('usado', "Usado em Consultas", 'right');

        $column_cod_erp->enableAutoHide('400');
        ...
        $column_usado_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            //code here
            $class = ($value=='N') ? 'warning'      : 'success';
            $label = ($value=='N') ? 'Bloqueado'    : 'Ativo';
            $div = new TElement('span');
            $div->class="label label-{$class}";
            $div->style="width:120px; text-shadow:none; font-size:12px; font-weight:lighter";
            $div->add($label);
        ...
        $this->datagrid->addColumn($column_usado_transformed);

        $action_OnMarcaUsado = new TDataGridAction(array('CategoriaList', 'OnMarcaUsado'));
        $action_OnMarcaUsado->setUseButton(false);
        $action_OnMarcaUsado->setButtonClass('btn btn-default btn-sm');
        $action_OnMarcaUsado->setLabel("Ativar/Desativar");
        $action_OnMarcaUsado->setImage('fas:power-off #009688');
        $action_OnMarcaUsado->setField(self::$primaryKey);

        $this->datagrid->addAction($action_OnMarcaUsado);
        ...
        $btnShowCurtainFilters->setAction(new TAction(['CategoriaList', 'onShowCurtainFilters']), "Filtros");
        ...
        $button_atualizar->setAction(new TAction(['CategoriaList', 'onRefresh']), "Atualizar");
        ...
        $button_limpar_filtros->setAction(new TAction(['CategoriaList', 'onClearFilters']), "Limpar filtros");
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['CategoriaList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-csv #00b894' );
        $dropdown_button_exportar->addPostAction( "XLS", new TAction(['CategoriaList', 'onExportXls'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:file-excel #4CAF50' );
        $dropdown_button_exportar->addPostAction( "PDF", new TAction(['CategoriaList', 'onExportPdf'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-pdf #e74c3c' );
        $dropdown_button_exportar->addPostAction( "XML", new TAction(['CategoriaList', 'onExportXml'],['static' => 1]), 'datagrid_'.self::$formName, 'far:file-code #95a5a6' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Gerencia","Categoria"]));
        }
    ...
    public function OnMarcaUsado($param = null) 
    {
        try 
        {
            ...
            if(isset($param['id'])){
                ...
                $oCategoria = Categoria::find($param['id']);
                if ($oCategoria)
                {
                    $oCategoria->usado = $oCategoria->usado == 'S' ? 'N' : 'S';
                    $oCategoria->store();
                }
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'CategoriaListSearch');
            $page->setProperty('page_name', 'CategoriaListSearch');

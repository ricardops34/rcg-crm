...
- [Técnicas Avançadas](guides/theme-service#advanced)
    ...
    - [Reset para Tema Padrão](guides/theme-service#advancedReset)
    - [Persistência de Tema](guides/theme-service#advancedPersist)
    - [Gerenciamento de Tamanhos](guides/theme-service#advancedSize)
- [Mais informações](guides/theme-service#more)
...
> **Recomendação:** Utilize sempre o serviço `PoThemeService` para manipulação de temas. Reserve esta técnica para casos extremos.

**Quando usar?**  
- Necessidade de ajustes pontuais não cobertos pelos tokens padrão  
- Prototipagem rápida  

**Implementação:**

```html
<html class='override-theme'>
```
...
Restaura todos os valores para o tema base configurado:

```typescript
// Em qualquer componente/service:
resetarTema() {
  this.poThemeService.resetBaseTheme();
}
```

<a id="advancedPersist"></a>
...
Padrão acessível (AAA):

```typescript
  // Habilitar tamanhos para small
...
- [PoThemeService API](documentation/po-theme) - Métodos completos do serviço
- [Animalia DS](https://doc.animaliads.io/) - Base dos nossos tokens de design
